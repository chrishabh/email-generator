...
class uploadedAndDownloadFileName extends Model
{
    ...
    static function getStatusOfEmailVerification($fileId){
        ...
        $is_failed_job = DB::select("SELECT CASE WHEN EXISTS ( SELECT 1 FROM failed_jobs f WHERE f.uuid = u.job_id ) THEN 1 ELSE 0 END AS is_failed FROM uploaded_and_download_file_names u WHERE u.id =$fileId");
